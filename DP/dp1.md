...
## [蓝](https://ac.nowcoder.com/acm/contest/116721/F)(计数，前缀和优化？？？)

$m$非常大，要直接安排位置十分困难，但注意到限制只有班级有没有人。考虑先dp出满足限制下有$x$个教室有人的方案数。用$L[0/1][r]$表示$c$类限制，右边界为$r$的最小l。考虑$f_{i,j}$表示前$i$个位置有$j$个为空的，且第$i$个位置为空的数量，$g_{i,j}$表示前$i$个位置有$j$个为空的，且第$i$个位置为非空的数量。注意到$f$只用满足$c=1$的限制，$g$只用满足$c=0$的限制
