...
令$F(x,y)$为恰好**没用**了$x$种颜色，$y$行，每列都有染色的方案数，$G(x,y)$为选出$x$种颜色**不用**，$y$行**不用**，其余随便的方案数，那么有
$$
G(x,y)=\binom{c}{x}\binom{n}{y} ((c+1-x)^{n-y}-1)^{m}
$$
由二项式反演有
$$
F(x,y)=\sum_{i\ge x}^{c}\sum_{j\ge y}^{n} (-1)^{i+j-x-y} \binom{i}{x}\binom{j}{y} G(i,j)
$$

$$
F(x,y)=\sum_{i\ge x}^{c}\sum_{j\ge y}^{n} (-1)^{i+j-x-y} \binom{i}{x}\binom{j}{y}\binom{c}{i}\binom{n}{j} ((c+1-i)^{n-j}-1)^m
$$

$$
F(0,0)=\sum_{i\ge 0}^{c}\sum_{j\ge 0}^{n} (-1)^{i+j} \binom{c}{i}\binom{n}{j} ((c+1-i)^{n-j}-1)^m
$$

虽然这个东西看上去比较复杂，但实际上完全不用动脑子。



## [Permutation](https://ac.nowcoder.com/acm/contest/108306/G)(笛卡尔树)

我们需要找到一些链，元素单调递减，且其能维持最小的区间是慢慢缩小的。我们可以在小根堆笛卡尔树上分析。注意到元素都不同，那么显然可以枚举最后一个元素，从而可以找到可以找到一条链，a就在这条链里面取数，假设最后一个元素的子树大小为$sz$，深度为$dep$，那么数组长度为$[1,sz]$ ，那么显然有方案数为



## [J. Moving on the Plane](https://qoj.ac/contest/2513/problem/14310)（曼哈顿距离转切比雪夫距离，最少->恰好）

先将曼哈顿距离转为切比雪夫距离，然后就可以单独考虑横纵坐标。就转化成在数轴上有$n$个点，$m$轮操作，每轮左移或右移，最后最大距离小于等于$K$的方案数。

考虑计算$f_{l,r}$表示在最终在$[l,r]$的方案，但这样会重复，考虑$f[l,r]-f[l+1，r]$得到最小值在$l$的方案数目。具体地用$dp_x$表示最小值为$x$的合法方案数，$dp_x=f_{x,x+k}-f_{x+1,x+k}$。而对每个点可以独立计算。



## [构造序列](https://ac.nowcoder.com/acm/contest/117762/E)

求$a_1+a_2+ \cdots +a_k =n$，$a_i>0$，$a_{i}>a_{i-1}$的方案数量。

注意到$b_i=a_i-(i-1)>0$，且$b$和$a$构成双射。

那么有
$$
\sum_{i=1}^{k} b_i =\sum_{i=1}^{k} a_i -\frac{k(k-1)}{2} =n-\frac{k(k-1)}{2}
$$
$b_1\ge 1,b_{i}\ge b_{i-1}$

全部减去1，有
$$
\sum_{i=1}^{k} =n-\frac{k(k+1)}{2}
$$
$b_1\ge 0,b_{i}\ge b_{i-1}$

下面考虑有解的情况。

这是将$n$拆分成不超多$k$项的拆分数，等价于拆成若干部分，每部分$\le k$的拆分数。显然其生成函数为
$$
\prod _{i=1}^{k} \frac{1}{1- x^i}
$$
